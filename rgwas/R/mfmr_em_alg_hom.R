mfmr_em_alg_hom	<- function( Yb, Y, X, trace, maxit=1e3, tol=1e-3 ){
	...
	for( it in 1:maxit ){
		...
		Lam_qerr	<- ( Y - Xalpha[,-(1:B)] ) %*% Lambda_qq
	...
	ll	<- ll_fxn_hom( Yb, Y, Xalpha, Sigma, N, P, B )

	list( pvec=NA, alpha=alpha, beta=NA, Sigma=Sigma, ll=ll, niter=it, conv=it<maxit )
}

ll_fxn_hom	<- function( Yb, Y, Xalpha, Sigma, N, P, B ){
